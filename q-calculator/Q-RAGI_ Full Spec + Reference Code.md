---
slug: q-ragi-full-spec-reference-code
status: draft
created: '2026-03-20'
updated: '2026-03-20'
version: 0.1.0
...
- multiplicity
traceability:
  path: 02-implementations/q-calculator/Q-RAGI_ Full Spec + Reference Code.md
  last_synced: '2026-03-20T17:17:15.239653Z'
---

Q-RAGI: Contractive LTV Affine Core — Full Spec
and Reference Code
Bottom line: single inequality drives everything. Design so qt := ∥Ξ(t)∥ + ∥Λop
                                                                             m (t)∥ LT < 1             for all t.
...
1) Objects and norms
      ...
      • Operator mix: Ξ(t) = ∑p∈Pt wp (t)Bp (t),            ∥Ξ(t)∥ ≤ ∑p∈Pt bp (t) ∣wp (t)∣.
...
Xt+1 = P (Ξ(t)Xt + Λop
                    m (t) T (Xt ) + Gt ), where Gt ∈ H is exogenous input (can be 0 ). If you omit P ,
take it as identity.


3) Contraction condition and guarantees
Define per‑step Lipschitz constant qt := ∥Ξ(t)∥ + ∥Λop
                                                    m (t)∥ LT . Assume supt qt ≤ q < 1 . Then for any
two trajectories Xt , Yt :


      • Incremental contraction: ∥Xt+1 − Yt+1 ∥ ≤ q ∥Xt − Yt ∥ . Hence ∥Xt − Yt ∥ ≤ q t ∥X0 − Y0 ∥ .
                                                             t
      • ISS bound with input Gt : ∥Xt ∥ ≤ q t ∥X0 ∥ + 1−q supτ ∥Gτ ∥ .
                                                        1−q
      • Stationary limit: if Ξ(t) → Ξ , Λm (t) → Λ , Gt → G , then Xt → X ∗ solving X ∗ = P (ΞX ∗ +
                                         op
               ∗
        ΛT (X ) + G) . Banach applies because the self‑map has Lipschitz ≤ q < 1 .
...
Goal: enforce ∥Ξ(t)∥ ≤ 1 − ε by construction.
      ...
      • Safety set at step t : St := {w : ∑
                                              p∈Pt bp (t) ∣wp ∣ ≤ 1 − ε},   ε ∈ (0, 1).



                                                        1
                                          ~ : min 1 ∥w − w
        • Projection problem for proposal w              ~ ∥2 s.t. ∑ b (t)∣w ∣ ≤ 1 − ε.
                                           t     w 2      t 2       p p     p
                                                                 ~ ) max (∣w
        • KKT soft‑threshold solution: for some λ ≥ 0 wp∗ = sign(w         ~ ∣ − λ b (t), 0). Choose λ =
                                                                  p         p       p
         0 if feasible; else pick the unique λ making the constraint tight. Then ∥Ξ(t)∥ ≤ ∑p bp (t)∣wp∗ ∣ ≤
         1 − ε.
        • Log primal/dual residuals each step for audit.


5) Continuous‑time extension (optional)
Ẋ = F (τ )X + Λop  m (τ )T (X) + g(τ ) . For contraction you need a logarithmic‑norm bound with respect to
the    chosen       norm:      μ(F (τ )) + ∥Λop
                                              m (τ )∥ LT ≤ −α < 0 a.e. τ .      Then   ∥X(τ ) − Y (τ )∥ ≤
 −α(τ −τ0 )
e           ∥X(τ0 ) − Y (τ0 )∥ .
