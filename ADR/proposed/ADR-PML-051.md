# ADR-PML-051: Documented Lean theorems missing in the `bindu-rta` subsystem (3 gaps)
...
- Tractability = **1.0**
- **Score = 12.0**  (cluster rank 8 of 11)
...
### Stated intent (documents)
  - docs/adr/completed/ADR-043-Rta-Morphism-and-Bindu.md:166 — asserts `FittingCheck` exists / is verified
  - docs/adr/completed/ADR-043-Rta-Morphism-and-Bindu.md:185 — asserts `BinduAttractor` exists / is verified
  - docs/adr/completed/ADR_041_ALP_CNL_Production_Rta_Metric.md:48 — asserts `RtaMetric` exists / is verified

### Implementation reality (lean/)
  - `FittingCheck` not found among 6978 lean declarations
  - `BinduAttractor` not found among 6978 lean declarations
  - `RtaMetric` not found among 6978 lean declarations
...
## Actionable Levers
1. Discharge the `sorry` in the existing `bindu-rta` declaration; add a `lake build` regression test proving the theorem.
