# ADR-PML-064: Documented Lean theorems missing in the `governance` subsystem (1 gaps)
...
- Tractability = **1.0**
- **Score = 4.0**  (cluster rank 10 of 10)
...
### Stated intent (documents)
  - docs/adr/completed/ADR-077-PIRTM-Fock-Space-Constitutional-Contractivity.md:13 — asserts `FockTrunc` exists / is verified

### Implementation reality (lean/)
  - `FockTrunc` not found among 6978 lean declarations
...
## Actionable Levers
1. Discharge the `sorry` in the existing `governance` declaration; add a `lake build` regression test proving the theorem.
