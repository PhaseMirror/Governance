# ADR-PML-063: Documented Lean theorems missing in the `sigma` subsystem (1 gaps)
...
- Tractability = **1.0**
- **Score = 4.0**  (cluster rank 9 of 10)
...
### Stated intent (documents)
  - docs/adr/completed/ADR-075-ORF-Coherence-Stratification-Kernel.md:74 — asserts `JensenShannonMetric` exists / is verified

### Implementation reality (lean/)
  - `JensenShannonMetric` not found among 6978 lean declarations
...
## Actionable Levers
1. Discharge the `sorry` in the existing `sigma` declaration; add a `lake build` regression test proving the theorem.
